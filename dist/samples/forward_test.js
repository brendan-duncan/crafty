...
function createFallbackCubemap(device) {
    ...
    for (let i = 0; i < 6; i++) {
        device.queue.writeTexture({ texture, origin: { x: 0, y: 0, z: i } }, data, { bytesPerRow: 8 }, { width: 1, height: 1 });
    }
    return texture;
}
async function main() {
    ...
    device.queue.writeTexture({ texture: brdfTex }, new Uint16Array([0x3C00, 0x0000, 0, 0]), // r=1.0, g=0.0
    { bytesPerRow: 8 }, { width: 1, height: 1 });
    // Create materials
