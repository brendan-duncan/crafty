...
function createControlPanel(effects, onChange) {
...
async function main() {
    ...
    let ibl = await computeIblGpu(device, skyTexture.gpuTexture);
    ...
    let snowPass = null;
    let ssgiPass = null;
    let autoExposurePass = null;
    let skinnedGeometryPass = null;
    let pointSpotShadowPass = null;
    let pointSpotLightPass = null;
    let graph;
    ...
    const effectsPanel = createControlPanel(effects, (key) => {
        if (key === 'ssao')
            return; // handled per-frame via updateParams
        if (key === 'ssgi')
            return; // handled per-frame via updateSettings
        if (key === 'shadows')
            return; // handled per-frame via updateLight
        if (key === 'aces')
            return; // handled per-frame via updateParams
        if (key === 'ao_dbg')
            return; // handled per-frame via updateParams
