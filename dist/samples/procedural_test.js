...
class ProceduralMaterial extends Material {
    ...
    animSpeed = 1.0;
    edgeWidth = 0.04;
    static _bglCache = new WeakMap();
    _device = null;
    _buffer = null;
    _bindGroup = null;
    _dirty = true;
    getShaderCode(_passType) {
    ...
    _data = new Float32Array(ALIGNMENT / 4);
    update(queue) {
...
async function main() {
    ...
    async function render() {
        ...
        cameraControls.update(fakeGO, dt);
        ...
        const items = [
            ...
            {
                mesh: cubeMesh,
                modelMatrix: Mat4.translation(0, 1.5, 0).multiply(Mat4.rotationY(time * 40 * Math.PI / 180)),
                normalMatrix: Mat4.identity(),
                material: matB,
            },
            {
                mesh: sphereMesh,
                modelMatrix: Mat4.translation(Math.sin(time * 0.6) * spread, 1.5, Math.cos(time * 0.6) * spread).multiply(Mat4.rotationY(time * 60 * Math.PI / 180)),
                normalMatrix: Mat4.identity(),
                material: matC,
            },
            {
                mesh: coneMesh,
                modelMatrix: Mat4.translation(Math.sin(time * 0.6 + Math.PI) * spread, 1.5, Math.cos(time * 0.6 + Math.PI) * spread),
                normalMatrix: Mat4.identity(),
                material: matA,
            },
        ];
        ...
        matC.markDirty();
        // Shadow draw items
        const shadowItems = items.map((item) => ({
            mesh: item.mesh,
