...
export class Chunk {
    ...
    static CUBE_VERTS = new Float32Array([
    ...
    generateVertices(neighbors) {
        ...
        const setBit = (x, y, z, face) => {
            drawnFaces[(x * H + y) * 6 + face] |= (1 << z);
        };
        const checkBit = (x, y, z, face) => {
            return (drawnFaces[(x * H + y) * 6 + face] & (1 << z)) !== 0;
        };
        const getType = (x, y, z) => padded[(x + 1) + (y + 1) * PW + (z + 1) * PWPH];
        const skipCheck = (b1, b2) => {
     ...
     */
    generateBlocks(seed, getErosion) {
        const W = Chunk.CHUNK_WIDTH;
        const H = Chunk.CHUNK_HEIGHT;
        const D = Chunk.CHUNK_DEPTH;
        ...
        for (let z = 0; z < D; z++) {
            for (let y = 0; y < H; y++) {
                for (let x = 0; x < W; x++) {
                    ...
                    const surfaceHeight = Math.abs(perlinNoise3Seed(g_x / 256.0, g_y / 512.0, g_z / 256.0, 0, 0, 0, seed) * colHeightMult[ci])
                    ...
                    if (g_y < surfaceHeight) {
                        if (Chunk._isCave(g_x, g_y, g_z, seed, surfaceHeight - g_y)) {
                            ...
                            this.setBlock(x, y, z, this._generateBlockBasedOnBiome(colBiome1[ci], colBiome2[ci], colBiomeBlend[ci], g_x, g_y, g_z, surfaceHeight));
                        }
                    }
    ...
    _generateAdditionalBlocks(p_x, p_y, p_z, _p_gX, p_gY, _p_gZ, _seed) {
    ...
    _generateBlockBasedOnBiome(biome1, biome2, blend, p_x, p_y, p_z, surfaceHeight) {
    ...
    static _determineBiomeFromNoise(temperature) {
        if (temperature > 0.35) {
    ...
    static _determineBiome(p_x, _p_y, p_z, seed) {
    ...
    static _isCave(gx, gy, gz, seed, depthBelowSurface) {
