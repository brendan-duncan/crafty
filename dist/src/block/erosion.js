...
function _sampleBaseHeight(gx, gz, seed) {
    const cont = perlinNoise3Seed(gx / 2048.0, 10, gz / 2048.0, 0, 0, 0, seed);
    const heightMult = Math.abs(perlinNoise3Seed(gx / 1024.0, 0, gz / 1024.0, 0, 0, 0, seed) * 450) *
...
function _sampleGradient(heights, size, px, py) {
    const x = px | 0;
    const y = py | 0;
    const fx = px - x;
    const fy = py - y;
...
function _erode(heights, size, seed) {
 ...
 */
export function buildErosionRegion(regionX, regionZ, seed) {
