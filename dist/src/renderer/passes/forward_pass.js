...
export class ForwardPass extends RenderPass {
    ...
    _modelBGL;
    _lightingIblBGL;
    _opaquePipelineCache = new Map();
    _transparentPipelineCache = new Map();
    _cameraBuffer;
    _cameraBindGroup;
    _lightingBuffer;
    ...
    _pointLightsBuffer;
    _spotLightsBuffer;
    _lightingIblBindGroup;
    _shadowMapArray;
    _pointShadowCubeArray;
    _opaqueItems = [];
    _transparentItems = [];
    /**
    ...
    _lightingScratch = new Uint32Array(4);
    _directionalScratch = new ArrayBuffer(DIRECTIONAL_LIGHT_SIZE);
    _directionalScratchF = new Float32Array(this._directionalScratch);
    _directionalScratchU = new Uint32Array(this._directionalScratch);
    _pointLightsScratch = new ArrayBuffer(POINT_LIGHT_SIZE * MAX_POINT_LIGHTS);
    _pointLightsScratchF = new Float32Array(this._pointLightsScratch);
    _pointLightsScratchU = new Uint32Array(this._pointLightsScratch);
    _spotLightsScratch = new ArrayBuffer(SPOT_LIGHT_SIZE * MAX_SPOT_LIGHTS);
    _spotLightsScratchF = new Float32Array(this._spotLightsScratch);
    _spotLightsScratchU = new Uint32Array(this._spotLightsScratch);
    _depthTexture;
    _depthView;
    _outputTexture;
    _outputView;
    ...
    _modelBindGroups = [];
    _bufferIndex = 0;
    constructor(cameraBGL, modelBGL, lightingIblBGL, cameraBuffer, cameraBindGroup, lightingBuffer, directionalLightBuffer, pointLightsBuffer, spotLightsBuffer, lightingIblBindGroup, shadowMapArray, pointShadowCubeArray, depthTexture, depthView, outputTexture, outputView) {
        super();
     ...
     */
    copyShadowMapToArray(encoder, sourceTexture, arrayLayer, size = 2048) {
        encoder.copyTextureToTexture({ texture: sourceTexture }, { texture: this._shadowMapArray, origin: { x: 0, y: 0, z: arrayLayer } }, { width: size, height: size, depthOrArrayLayers: 1 });
    }
    /**
     ...
     */
    updateLights(ctx, directionalLight, pointLights, spotLights) {
    ...
    _drawItem(pass, ctx, item, transparent) {
    ...
    _getPipeline(device, material, transparent) {
    ...
    _getOrCreateModelBuffer(device) {
    ...
    _getOrCreateModelBindGroup(device) {
        const idx = this._bufferIndex++ % 32;
