...
export class GodrayPass extends RenderPass {
    ...
    _blurVParamsBuf;
    _compParamsBuf;
    constructor(fogA, fogAView, fogB, fogBView, hdrView, marchPipeline, blurHPipeline, blurVPipeline, compositePipeline, marchBG, blurHBG, blurVBG, compositeBG, marchParamsBuf, blurHParamsBuf, blurVParamsBuf, compParamsBuf) {
        super();
        this._fogA = fogA;
    ...
    static create(ctx, gbuffer, shadowPass, hdrView, cameraBuffer, lightBuffer) {
        ...
        device.queue.writeBuffer(blurHParamsBuf, 0, new Float32Array([1.0, 0.0, 0.0, 0.0]).buffer); // blur_dir=(1,0)
        const blurVParamsBuf = device.createBuffer({
            label: 'GodrayBlurVParams', size: PARAMS_SIZE,
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
        });
        device.queue.writeBuffer(blurVParamsBuf, 0, new Float32Array([0.0, 1.0, 0.0, 0.0]).buffer); // blur_dir=(0,1)
        // ---- Composite param buffer ----
        const compParamsBuf = device.createBuffer({
            label: 'GodrayCompositeParams', size: PARAMS_SIZE,
            usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
        });
        device.queue.writeBuffer(compParamsBuf, 0, new Float32Array([0.0, 0.0, 2.0, 0.0]).buffer); // fog_curve at [2]
        ...
        return new GodrayPass(fogA, fogAView, fogB, fogBView, hdrView, marchPipeline, blurHPipeline, blurVPipeline, compositePipeline, marchBG, blurHBG, blurVBG, compositeBG, marchParamsBuf, blurHParamsBuf, blurVParamsBuf, compParamsBuf);
    }
    /**
    ...
    updateParams(ctx) {
        ctx.queue.writeBuffer(this._marchParamsBuf, 0, new Float32Array([this.scattering, this.maxSteps, 0.0, 0.0]).buffer);
        // compParamsBuf layout: blur_dir(vec2) + fog_curve(f32) + _pad
        // blur_dir is (0,0) here — unused by composite — fog_curve is at offset 8.
        ctx.queue.writeBuffer(this._compParamsBuf, 0, new Float32Array([0.0, 0.0, this.fogCurve, 0.0]).buffer);
    }
    /**
