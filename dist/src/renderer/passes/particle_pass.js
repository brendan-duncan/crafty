...
export class ParticlePass extends RenderPass {
    ...
    _cuBuf = new Float32Array(COMPUTE_UNI_SIZE / 4);
    _cuiView = new Uint32Array(this._cuBuf.buffer);
    _camBuf = new Float32Array(CAMERA_UNI_SIZE / 4);
    _hmUniBuf = new Float32Array(HEIGHTMAP_UNI_SIZE / 4);
    _hmRes = new Uint32Array([HEIGHTMAP_RES]);
    _resetArr = new Uint32Array(1); // stays 0
    constructor(gbuffer, hdrView, isForward, config, maxParticles, particleBuffer, aliveList, counterBuffer, indirectBuffer, computeUniforms, renderUniforms, cameraBuffer, spawnPipeline, updatePipeline, compactPipeline, indirectPipeline, renderPipeline, computeDataBG, computeUniBG, compactDataBG, compactUniBG, renderDataBG, cameraRenderBG, renderParamsBG, heightmapDataBuf, heightmapUniBuf, heightmapBG) {
        super();
     ...
     */
    static create(ctx, config, gbuffer, hdrView) {
        const { device } = ctx;
        ...
        device.queue.writeBuffer(particleBuffer, 0, initData.buffer);
        ...
        return new ParticlePass(gbuffer, hdrView, isForward, config, maxParticles, particleBuffer, aliveList, counterBuffer, indirectBuffer, computeUniforms, renderUniforms, cameraBuffer, spawnPipeline, updatePipeline, compactPipeline, indirectPipeline, renderPipeline, computeDataBG, computeUniBG, compactDataBG, compactUniBG, renderDataBG, cameraRenderBG, isForward ? undefined : renderParamsBG, heightmapDataBuf, heightmapUniBuf, heightmapBG);
    }
    /**
     ...
     */
    updateHeightmap(ctx, heights, originX, originZ, extent) {
        if (!this._heightmapDataBuf || !this._heightmapUniBuf) {
            return;
        }
        ctx.queue.writeBuffer(this._heightmapDataBuf, 0, heights.buffer);
        const uni = this._hmUniBuf;
        uni[0] = originX;
        uni[1] = originZ;
        uni[2] = extent;
        // Write first 3 floats then the u32 resolution separately to avoid aliasing.
        ctx.queue.writeBuffer(this._heightmapUniBuf, 0, uni.buffer, 0, 12);
     ...
     */
    update(ctx, dt, view, proj, viewProj, invViewProj, camPos, near, far, worldTransform) {
        this._time += dt;
        ...
        cu[18] = cb;
        cu[19] = ca;
        ctx.queue.writeBuffer(this._computeUniforms, 0, cu.buffer);
        ...
        camData[67] = near;
        camData[68] = far;
        ctx.queue.writeBuffer(this._cameraBuffer, 0, camData.buffer);
    }
    /**
