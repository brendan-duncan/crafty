...
export class BlockShadowPass extends RenderPass {
    ...
    _modelBGL;
    _atlasBG;
    _orientBG_X; // right=(1,0,0) for XY-plane quads
    _orientBG_Z; // right=(0,0,1) for ZY-plane quads
    _chunks = new Map();
    _cascades = [];
    constructor(device, shadowMapArrayViews, pipeline, transparentPipeline, propPipeline, cascadeBGs, cascadeBuffers, modelBGL, atlasBG, orientBG_X, orientBG_Z) {
        super();
        this._device = device;
     ...
     */
    update(ctx, cascades, camX, camZ) {
        this._cascades = cascades;
        this._camX = camX;
        this._camZ = camZ;
        const n = Math.min(cascades.length, this._cascadeBuffers.length);
        for (let i = 0; i < n; i++) {
            ctx.queue.writeBuffer(this._cascadeBuffers[i], 0, cascades[i].lightViewProj.data.buffer);
        }
    }
    /**
