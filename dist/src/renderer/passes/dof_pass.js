...
export class DofPass extends RenderPass {
    ...
    _compositePipeline;
    _uniformBuffer;
    _prefilterBG; // group 0: uniforms + hdr + depth + sampler
    _compBG0; // group 0 for composite: uniforms + hdr + sampler (no depth)
    _compBG1; // group 1 for composite: half_tex
    constructor(result, resultView, half, halfView, prefilterPipeline, compositePipeline, uniformBuffer, prefilterBG, compBG0, compBG1) {
        super();
        this._result = result;
     ...
     */
    static create(ctx, hdrView, depthView) {
        ...
        device.queue.writeBuffer(uniformBuffer, 0, new Float32Array([30.0, 60.0, 6.0, 0.1, 1000.0, 0, 0, 0]).buffer);
        ...
        return new DofPass(result, resultView, half, halfView, prefilterPipeline, compositePipeline, uniformBuffer, prefilterBG, compBG0, compBG1);
    }
    /**
     ...
     */
    updateParams(ctx, focusDistance = 30.0, focusRange = 60.0, bokehRadius = 6.0, near = 0.1, far = 1000.0, blurScale = 1.0) {
        ctx.device.queue.writeBuffer(this._uniformBuffer, 0, new Float32Array([focusDistance, focusRange, bokehRadius, near, far, blurScale, 0, 0]).buffer);
    }
    /**
