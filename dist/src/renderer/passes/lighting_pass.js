...
export class DeferredLightingPass extends RenderPass {
    ...
    _sceneBindGroup;
    _gbufferBindGroup;
    _aoBindGroup; // group 2: AO + SSGI combined
    _iblBindGroup; // group 3: irradiance cube, prefilter cube, BRDF LUT
    _defaultCloudShadow;
    _defaultSsgi;
    _defaultIblCube;
    _defaultBrdfLut;
    ...
    _cameraScratch = new Float32Array(CAMERA_SIZE / 4);
    _lightScratch = new Float32Array(LIGHT_SIZE / 4);
    _lightScratchU = new Uint32Array(this._lightScratch.buffer);
    _cloudShadowScratch = new Float32Array(3);
    constructor(hdrTexture, hdrView, pipeline, sceneBindGroup, gbufferBindGroup, aoBindGroup, iblBindGroup, cameraBuffer, lightBuffer, defaultCloudShadow, defaultSsgi, defaultIblCube, defaultBrdfLut, device, aoBGL, aoView, aoSampler, ssgiSampler) {
        super();
     ...
     */
    static create(ctx, gbuffer, shadowPass, aoView, cloudShadowView, iblTextures) {
        ...
        device.queue.writeTexture({ texture: defaultCloudShadow }, new Uint8Array([255]), { bytesPerRow: 1 }, { width: 1, height: 1 });
        ...
        return new DeferredLightingPass(hdrTexture, hdrView, pipeline, sceneBindGroup, gbufferBindGroup, aoBindGroup, iblBindGroup, cameraBuffer, lightBuffer, cloudShadowView ? null : defaultCloudShadow, defaultSsgi, iblTextures ? null : defaultIblCube, iblTextures ? null : defaultBrdfLut, device, aoBGL, aoView, aoSampler, ssgiSampler);
    }
    /**
    ...
    updateCamera(ctx, view, proj, viewProj, invViewProj, camPos, near, far) {
        ...
        data[67] = near;
        data[68] = far;
        ctx.queue.writeBuffer(this.cameraBuffer, 0, data.buffer);
    }
    /**
     ...
     */
    updateLight(ctx, dir, color, intensity, cascades, shadowsEnabled = true, debugCascades = false, shadowSoftness = 0.02) {
        ...
        ctx.queue.writeBuffer(this.lightBuffer, 0, data.buffer);
    }
    /**
     ...
     */
    updateCloudShadow(ctx, originX, originZ, extent) {
        const data = this._cloudShadowScratch;
        data[0] = originX;
        data[1] = originZ;
        data[2] = extent;
        ctx.queue.writeBuffer(this.lightBuffer, 312, data.buffer);
    }
    /**
