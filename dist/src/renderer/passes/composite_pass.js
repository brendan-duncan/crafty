...
export class CompositePass extends RenderPass {
    ...
    _pipeline;
    _bg0; // textures + sampler
    _bg1; // camera + light (shared buffers)
    _bg2; // params + stars + exposure
    _paramsBuf;
    _starBuf;
    _paramsAB = new ArrayBuffer(PARAMS_SIZE);
    _paramsF = new Float32Array(this._paramsAB);
    _paramsU = new Uint32Array(this._paramsAB);
    _starScratch = new Float32Array(STAR_UNI_SIZE / 4);
    constructor(pipeline, bg0, bg1, bg2, paramsBuf, starBuf) {
        super();
     ...
     */
    static create(ctx, hdrView, aoView, depthView, cameraBuffer, lightBuffer, exposureBuf) {
     ...
     */
    updateParams(ctx, isUnderwater, uwTime, aces, debugAO, hdrCanvas) {
     ...
     */
    updateStars(ctx, invViewProj, camPos, sunDir) {
        ...
        data[22] = sunDir.z;
        data[23] = 0;
        ctx.queue.writeBuffer(this._starBuf, 0, data.buffer);
    }
    /**
