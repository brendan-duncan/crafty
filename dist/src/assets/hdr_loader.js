...
export function parseHdr(buffer) {
    ...
    function readOldScanline(y, r0, g0, b0, e0) {
        const base = y * width * 4;
        data[base + 0] = r0;
        data[base + 1] = g0;
        data[base + 2] = b0;
...
function getOrCreateDecodeResources(device) {
 ...
 */
export async function createHdrTexture(device, hdr) {
    ...
    device.queue.writeTexture({ texture: srcTex }, data.buffer, { bytesPerRow: width * 4 }, { width, height });
