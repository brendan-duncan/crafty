...
export class Mesh {
     ...
     */
    static fromData(device, vertices, indices) {
        const vb = device.createBuffer({
            label: 'Mesh VertexBuffer',
            size: vertices.byteLength,
            usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST,
        });
        device.queue.writeBuffer(vb, 0, vertices.buffer, vertices.byteOffset, vertices.byteLength);
        const ib = device.createBuffer({
            label: 'Mesh IndexBuffer',
            size: indices.byteLength,
            usage: GPUBufferUsage.INDEX | GPUBufferUsage.COPY_DST,
        });
        device.queue.writeBuffer(ib, 0, indices.buffer, indices.byteOffset, indices.byteLength);
     ...
     */
    static createCube(device, size = 1) {
        const h = size / 2;
     ...
     */
    static createSphere(device, radius = 0.5, latSegments = 32, lonSegments = 32) {
        const vertData = [];
        const idxData = [];
        for (let lat = 0; lat <= latSegments; lat++) {
            ...
            for (let lon = 0; lon <= lonSegments; lon++) {
                ...
                vertData.push(nx * radius, ny * radius, nz * radius, nx, ny, nz, lon / lonSegments, lat / latSegments, -sinPhi, 0, cosPhi, 1);
            }
        }
        for (let lat = 0; lat < latSegments; lat++) {
            for (let lon = 0; lon < lonSegments; lon++) {
     ...
     */
    static createCone(device, radius = 0.5, height = 1.0, segments = 16) {
        const vertData = [];
        const idxData = [];
     ...
     */
    static createPlane(device, width = 10, depth = 10, segX = 1, segZ = 1) {
        const vertData = [];
        const idxData = [];
