...
export class PointLight extends Component {
  ...
  cubeFaceViewProjs(near = 0.05): Mat4[] {
    ...
    const faces: Array<{ dir: Vec3; up: Vec3 }> = [
      { dir: new Vec3( 1,  0,  0), up: new Vec3(0, -1,  0) },
      { dir: new Vec3(-1,  0,  0), up: new Vec3(0, -1,  0) },
      { dir: new Vec3( 0,  1,  0), up: new Vec3(0,  0,  1) },
      { dir: new Vec3( 0, -1,  0), up: new Vec3(0,  0, -1) },
      { dir: new Vec3( 0,  0,  1), up: new Vec3(0, -1,  0) },
      { dir: new Vec3( 0,  0, -1), up: new Vec3(0, -1,  0) },
    ];
    return faces.map(({ dir, up }) =>
      proj.multiply(Mat4.lookAt(pos, pos.add(dir), up))
    );
  }
}
