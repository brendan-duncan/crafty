...
async function main(): Promise<void> {
  ...
  const sphereMat = new PbrMaterial({ albedo: [0.9, 0.2, 0.1, 1], roughness: 0.3, metallic: 0.8 });
  const planeMat = new PbrMaterial({ albedo: [0.3, 0.3, 0.35, 1], roughness: 0.7, metallic: 0.0 });
  ...
  function frame(): void {
    ...
    if (ctx.deltaTime > 0) {
      ...
      statsEl.textContent = `${smoothFps.toFixed(0)} fps | Deferred PBR Full`;
    }
    ...
    const skyOut = atmospherePass.addToGraph(graph);
