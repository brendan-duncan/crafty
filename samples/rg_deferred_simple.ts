...
async function main(): Promise<void> {
  ...
  function frame(): void {
    ...
    const cascades: CascadeData[] = [{
      lightViewProj,
      splitFar: 100,
      depthRange: 39,                   // far - near of the lightProj
      texelWorldSize: 20 / shadowMapSize, // (2 * 10) world-space units per texel
    }];
    ...
    lightingPass.updateLight(ctx, lightDir, { x: 1.0, y: 0.95, z: 0.9 }, 3.0, cascades, true, false);
    ...
    // GeometryPass writes the gbuffer (including HDR albedo). DeferredLightingPass
    // composites lighting into an HDR target. TonemapPass applies exposure + gamma
    // and blits to the swapchain (it must sample the HDR target, so an HDR
    // intermediate is required regardless of canvas format).
